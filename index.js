...
function getFinals(items) {
   const getAFinals = fifiaData.filter(item=>item.Stage=='Final')
   return getAFinals;
}
...
function getYears(fifaData) {
    return getFinals(fifaData).map(items => items.Year);
}
...
function getWinners(fifaData) {
    ...
    winners = winners.map(function(items){
...
function getAverageGoals(arrayCallBack) {
   let totalNumber = arrayCallBack.reduce(function(accumulator, items){
       return accumulator + items["Home Team Goals"] + item["Away Team Goals"];
   }, 0)
